...
/** Séparateur de zone avec label centré */
...
export default function AccueilPage() {
  ...
  return (
    ...
    >
      {/* ══════════════════════════════════════════
          ZONE 1 — Above fold
          Salutation + date hijri + carte prière hero + CTA primaire
      ══════════════════════════════════════════ */}
      ...
      {/* Salutation + date hijri sur une ligne compacte (non-kids) */}
      {ageMode !== "kids" && (
        <motion.div variants={itemVariants} className="flex items-center justify-between gap-3">
          {/* Salutation */}
          <div className="min-w-0">
            <p className="text-xs tracking-widest uppercase opacity-50 leading-none" style={{ color: "var(--text)", fontFamily: "var(--font-dm-sans)" }}>
              {tt("common.greeting")}
            </p>
            <h1 className="mt-0.5 text-xl font-bold truncate" style={{ color: "var(--text)", fontFamily: "var(--font-bricolage)" }}>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <div className="text-right">
              <p className="text-sm font-bold leading-none" style={{ color: "var(--gold)", fontFamily: "var(--font-amiri)" }}>
                {formatHijri(hijri)}
              </p>
              <p className="text-[10px] opacity-40 mt-0.5" style={{ color: "var(--text)", fontFamily: "var(--font-dm-sans)" }}>
                {hijri.monthFr} {hijri.year} H
                {hijri.isRamadan && " · Ramadan"}
                {hijri.isJumua && !hijri.isRamadan && " · Jumu'ah"}
              </p>
            </div>
      ...
      {/* ── EXPLORATEUR : carte prière compacte (sous le CTA jeu) ── */}
      {!isPratiquant && nextPrayer && times && (
        <motion.div variants={itemVariants} whileTap={{ scale: 0.985 }} transition={springTap}>
          <Link
            href="/prieres"
      ...
      {/* ── PRATIQUANT : CTA jeu discret (sous la carte prière) ── */}
      {isPratiquant && ageMode !== "kids" && ageMode !== "elder" && (
        <motion.div variants={itemVariants}>
          <Link href="/oasis">
            <motion.div
      ...
      {/* ── FALLBACK : onboarding pas complété ── */}
      ...
      {/* ══════════════════════════════════════════
          ZONE 2 — Premier scroll
          Motivateurs quotidiens : objectif, azkar, streak
      ══════════════════════════════════════════ */}
      <ZoneDivider label="Pratique du jour" />
      ...
      {/* ── OBJECTIF : explorer — défi famille + grille ── */}
      ...
      {/* ── PRATIQUANT : stats du jour (dhikr + tâches) — masquées si zéro ── */}
      {isPratiquant && ageMode !== "elder" && ageMode !== "kids" &&
        (stats.totalDhikr > 0 || stats.tasksTotal > 0) && (
      ...
      {/* ── EXPLORATEUR : stats du jour — masquées si zéro ── */}
      {!isPratiquant && ageMode !== "elder" && ageMode !== "kids" &&
        (stats.totalDhikr > 0 || stats.tasksTotal > 0) && (
        <motion.div variants={itemVariants} className="grid grid-cols-2 gap-3">
          {[
      ...
      {/* ══════════════════════════════════════════
          ZONE 3 — Contenu découverte
          Hadith, dhikr, mosquée, grande histoire, événements, raccourcis
      ══════════════════════════════════════════ */}
      <ZoneDivider label="Découverte" />
