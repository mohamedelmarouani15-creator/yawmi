...
import {
  ...
  MessageSquare, ChevronDown, ChevronUp, BookOpen, Dumbbell,
} from "lucide-react";
...
import PronunciationButton from "@/components/arabe/PronunciationButton";
import VoiceInput from "@/components/arabe/VoiceInput";
import PracticeMode from "@/components/arabe/PracticeMode";

interface ChatMsg { role: "user" | "prof"; text: string }

type Tab = "cours" | "pratique";
    ...
    <div className="flex flex-col min-h-screen pb-6" style={{ background: "#020a05" }}>
      ...
      <div className="sticky top-0 z-10 px-5 pt-safe-top pt-4 pb-3"
        ...
        <div className="flex gap-2">
          {(["cours", "pratique"] as Tab[]).map(t => {
            const Icon = t === "cours" ? BookOpen : Dumbbell;
            ...
            return (
              <motion.button
                ...
                style={{
                  ...
                  color: isActive ? meta.color : "rgba(248,244,236,0.4)",
                  fontFamily: "var(--font-dm-sans)",
                }}
              >
                <Icon size={12} />
                {t === "cours" ? "Cours" : "Pratique"}
              </motion.button>
            );
          })}
        </div>
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === "cours" && (
            ...
            >
              ...
              {/* Vocab with pronunciation + voice input */}
              <div className="rounded-2xl overflow-hidden"
                ...
                <button className="flex items-center justify-between w-full px-4 py-3"
                  ...
                  {showVocab
                    ? <ChevronUp size={14} style={{ color: "rgba(248,244,236,0.3)" }} />
                    : <ChevronDown size={14} style={{ color: "rgba(248,244,236,0.3)" }} />}
                </button>
                <AnimatePresence>
                  {showVocab && (
                        ...
                        {lesson.vocab.map((v, i) => (
                          <div key={i} className="flex items-center gap-3 rounded-xl px-3 py-2.5"
                            ...
                            <p className="text-xl shrink-0"
                              style={{ color: meta.color, fontFamily: "var(--font-amiri)", direction: "rtl", minWidth: 60, textAlign: "right" }}>
                              {v.arabic}
                            </p>
                            {/* Translation */}
                            <div className="flex-1 min-w-0">
                        ...
                        {lesson.examples.map((ex, i) => (
                          <div key={i} className="rounded-xl px-3 py-2.5"
                            ...
                            <div className="flex items-start gap-2">
                              <p className="text-lg flex-1 mb-0.5"
                                style={{ color: "var(--text)", fontFamily: "var(--font-amiri)", direction: "rtl", textAlign: "right" }}>
                                {ex.arabic}
                              </p>
                              {ex.arabic && (
                                <PronunciationButton
                  ...
                  {showResult && (
                    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
                      ...
                      <p className="text-xs font-bold mb-0.5"
                        style={{ color: correct ? "#22c55e" : "#f87171", fontFamily: "var(--font-dm-sans)" }}>
              ...
              <div className="flex flex-col gap-3">
                <motion.button whileTap={{ scale: 0.97 }} transition={springTap}
                  ...
                  style={{
                    background: chatOpen ? `${meta.color}18` : "rgba(255,255,255,0.04)",
                    border: `1px solid ${chatOpen ? meta.color + "50" : "rgba(255,255,255,0.1)"}`,
                  }}>
          ...
          {tab === "pratique" && (
            <motion.div
              ...
              className="pb-8"
            >
              <PracticeMode lesson={lesson} color={meta.color} />
            </motion.div>
          )}
        </AnimatePresence>
      </div>
