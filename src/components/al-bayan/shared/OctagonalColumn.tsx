...
interface OctagonalColumnProps {
  ...
  color?: string;
  accentColor?: string;
  /** Corridors n'ont aucune lumière à ombres (bougies non shadow-casting) —
   * les colonnes y sont désormais très nombreuses (galeries longues, cf.
   * brief "colonnades"), donc castShadow/receiveShadow n'y apportent aucun
   * rendu visible tout en coûtant un passage d'ombre réel dès qu'une
   * lumière à ombres d'une zone voisine déborde sur le seuil. Défaut à
   * `true` pour ne pas changer le rendu des colonnes déjà en place
   * (Vestibule, arcades de zone). */
  shadows?: boolean;
}
...
export default function OctagonalColumn({
  ...
  return (
    <group position={position}>
      <mesh geometry={baseGeo} material={accentMat} position={[0, 0.125, 0]} castShadow={shadows} receiveShadow={shadows} />
      <mesh geometry={shaftGeo} material={stoneMat} position={[0, height / 2, 0]} castShadow={shadows} receiveShadow={shadows} />
      <mesh geometry={capGeo} material={accentMat} position={[0, height - 0.15, 0]} castShadow={shadows} receiveShadow={shadows} />
    </group>
  );
}
