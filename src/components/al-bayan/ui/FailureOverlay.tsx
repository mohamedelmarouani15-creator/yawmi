...
const SOLUTIONS: Record<"A" | "B" | "C", { label: string; digit: number; explanation: string }> = {
  A: { label: "Le Poids du Témoignage", digit: 2, explanation: "Khuzayma ibn Thâbit, Dhûl-Shahâdatayn : son témoignage vaut celui de DEUX hommes." },
  B: { label: "Le Rasm Primitif", digit: 7, explanation: "فتبينوا (fa-tabayyanû) — Al-Hujurât 49:6 — compte SEPT lettres." },
  C: { label: "La Route des Codicilles", digit: 4, explanation: "QUATRE villes du consensus : Médine, Koufa, Bassora, Damas." },
};
...
export default function FailureOverlay() {
  ...
  const solvedEnigmas = (["A", "B", "C"] as const).filter((k) => enigmaStates[k].solved);
  const unsolvedEnigmas = (["A", "B", "C"] as const).filter((k) => !enigmaStates[k].solved);
      ...
      <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, ease: "easeOut" }}
        ...
        <div className="rounded-2xl p-4 w-full" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)" }}>
          <p style={{ fontSize: 11, fontFamily: "var(--font-dm-sans)", color: "rgba(248,244,236,0.6)", lineHeight: 1.7, textAlign: "center", fontStyle: "italic" }}>
            &quot;Les manuscrits se referment, mais la lettre ne périt jamais. Les copistes de Médine
            ont mis des années à fixer chaque point, chaque témoignage, chaque route. Vous avez
            commencé ce soir ce qu&apos;eux ont achevé en plusieurs vies. Revenez, et continuez leur œuvre.&quot;
          </p>
        </div>
          ...
          <div className="w-full">
            ...
            <div className="flex flex-col gap-2">
              {solvedEnigmas.map((key) => {
                ...
                return (
                  <div key={key} className="flex items-center gap-3 rounded-xl px-3 py-2" style={{ background: "rgba(52,211,153,0.08)", border: "1px solid rgba(52,211,153,0.2)" }}>
                    ...
                    <div>
                      <p style={{ fontSize: 10, color: "#34d399", fontFamily: "var(--font-dm-sans)", fontWeight: 700 }}>✓ {sol.label}</p>
                      <p style={{ fontSize: 9, color: "rgba(248,244,236,0.4)", fontFamily: "var(--font-dm-sans)" }}>{sol.explanation}</p>
                    </div>
                  </div>
                );
              })}
                ...
                <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} style={{ overflow: "hidden" }}>
                  <div className="flex flex-col gap-2">
                    {unsolvedEnigmas.map((key) => {
                      const sol = SOLUTIONS[key];
                      return (
                        <motion.div key={key} initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} transition={{ delay: 0.1 }}
                          ...
                          <div>
                            <p style={{ fontSize: 10, color: "rgba(212,175,55,0.7)", fontFamily: "var(--font-dm-sans)", fontWeight: 700 }}>{sol.label}</p>
                            <p style={{ fontSize: 9, color: "rgba(248,244,236,0.4)", fontFamily: "var(--font-dm-sans)" }}>{sol.explanation}</p>
                          </div>
                        </motion.div>
                      );
